...
class ServerWorld(
    ...
    private val _event = MutableSharedFlow<GameEvent>(extraBufferCapacity = 64) // добавил буфер для надежности
...
class SaveSystem {
    ...
    fun save(player: PlayerSave) {
        ...
        println("Сохранено: $text")
    }

    fun loadFile(playerId: String): PlayerSave? {
...
fun main() = KoolApplication {
    ...
    val playerId = "Oleg"
    val server = ServerWorld(initialPlayerId = playerId)
    ...
    addScene {
        ...
        coroutineScope.launch {
            server.events.collect { event ->
                when (event) {
                    is DamageDealt -> println("⚔${event.playerId} нанес ${event.amount} урона ${event.targetId}")
                    is QuestStateChanged -> println(" ${event.playerId} перешел на этап: ${event.newState} квеста ${event.questId}")
                    is PlayerProgressSaved -> println("${event.reason} для игрока ${event.playerId}")
                }
            }
        }

        coroutineScope.launch {
            server.playerState.collect { playerSave ->
                println("Текущее состояние: HP=${playerSave.hp}, Gold=${playerSave.gold}, Квесты=${playerSave.questStates}")
            }
        }

        coroutineScope.launch {
            println(" Начало демонстрации...")

            delay(3000)

            println("--- ШАГ 1: Сохранение ---")
            saver.save(server.playerState.value)

            delay(2000)

            println("--- ШАГ 2: Наносим урон ---")
            server.dealDamage(playerId, "Dragon", 30)
            println("Текущее HP после урона: ${server.playerState.value.hp}")

            delay(2000)

            println("--- ШАГ 3: Загрузка сохранения ---")
            val loadedSave = saver.loadFile(playerId)
            if (loadedSave != null) {
                println("Загруженные данные: HP=${loadedSave.hp}, Gold=${loadedSave.gold}")
                println("Применяем загруженное состояние...")
                server.applyLoaded(loadedSave)
            }

            delay(1000)
            println("--- РЕЗУЛЬТАТ ---")
            println("Финальное состояние: HP=${server.playerState.value.hp}")
        }

        println("Сцена инициализирована.")
    }
}